...
UDS_APP_ConnHandleNotEvt_t UDSHandleNotification;
ETS_APP_ConnHandleNotEvt_t ETSHandleNotification;
RCS_APP_ConnHandleNotEvt_t RCSHandleNotification;

static char a_GapDeviceName[] = {  'G', 'e', 'n', 'e', 'r', 'i', 'c', ' ', 'H', 'e', 'a', 'l', 't', 'h', ' ', 'S', 'e', 'n', 's', 'o', 'r' }; /* Gap Device Name */

/* Advertising Data */
uint8_t a_AdvData[28] =
{
  7, AD_TYPE_COMPLETE_LOCAL_NAME, 'G', 'H', 'S', '_', 'X', 'X',  /* Complete name */
...
/* USER CODE BEGIN PV */
#if(BLE_CFG_GHS_PTS == 1)
static uint8_t a_ServiceData[7] =
{
  6, AD_TYPE_SERVICE_DATA, 0x1C, 0x18, 0x01, 0x00, 0x00
};
static uint8_t a_ServiceDataUI2[9] =
{
  8, AD_TYPE_SERVICE_DATA, 0x1C, 0x18, 0x01, 0x00, 0x00, 0x01, 0x02
};
static uint8_t a_ServiceDataUI12[10] =
{
  9, AD_TYPE_SERVICE_DATA, 0x1C, 0x18, 0x01, 0x00, 0x00, 0x02, 0x01, 0x02
};
#endif
/* USER CODE END PV */
...
/* USER CODE BEGIN GV */
CurrentGapParam_t CurrentGapPram;
uint8_t iteration;
/* USER CODE END GV */
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification(void *p_Pckt)
{
  ...
  switch (p_event_pckt->evt)
  {
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      UDSHandleNotification.EvtOpcode = UDS_DISCON_HANDLE_EVT;
      ETSHandleNotification.EvtOpcode = ETS_DISCON_HANDLE_EVT;
      RCSHandleNotification.EvtOpcode = RCS_DISCON_HANDLE_EVT;
      ...
      UDSHandleNotification.ConnectionHandle = p_disconnection_complete_event->Connection_Handle;
      ETSHandleNotification.ConnectionHandle = p_disconnection_complete_event->Connection_Handle;
      RCSHandleNotification.ConnectionHandle = p_disconnection_complete_event->Connection_Handle;
      GHS_APP_EvtRx(&GHSHandleNotification);
      DIS_APP_EvtRx(&DISHandleNotification);
      BAS_APP_EvtRx(&BASHandleNotification);
      UDS_APP_EvtRx(&UDSHandleNotification);
      ETS_APP_EvtRx(&ETSHandleNotification);
      RCS_APP_EvtRx(&RCSHandleNotification);
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        case HCI_LE_CONNECTION_UPDATE_COMPLETE_SUBEVT_CODE:
        {
          ...
          CurrentGapPram.MinConInt = p_conn_update_complete->Conn_Interval;
          CurrentGapPram.MaxConInt = p_conn_update_complete->Conn_Interval;
          CurrentGapPram.Latency   = p_conn_update_complete->Conn_Latency;
          CurrentGapPram.SupervisionTimeoutMultiplier = p_conn_update_complete->Supervision_Timeout;
        ...
        case HCI_LE_ENHANCED_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          UDSHandleNotification.EvtOpcode = UDS_CONN_HANDLE_EVT;
          ETSHandleNotification.EvtOpcode = ETS_CONN_HANDLE_EVT;
          RCSHandleNotification.EvtOpcode = RCS_CONN_HANDLE_EVT;
          ...
          UDSHandleNotification.ConnectionHandle = p_enhanced_conn_complete->Connection_Handle;
          ETSHandleNotification.ConnectionHandle = p_enhanced_conn_complete->Connection_Handle;
          RCSHandleNotification.ConnectionHandle = p_enhanced_conn_complete->Connection_Handle;
          GHS_APP_EvtRx(&GHSHandleNotification);
          DIS_APP_EvtRx(&DISHandleNotification);
          BAS_APP_EvtRx(&BASHandleNotification);
          UDS_APP_EvtRx(&UDSHandleNotification);
          ...
          CurrentGapPram.MinConInt = p_enhanced_conn_complete->Conn_Interval;
          CurrentGapPram.MaxConInt = p_enhanced_conn_complete->Conn_Interval;
          CurrentGapPram.Latency   = p_enhanced_conn_complete->Conn_Latency;
          CurrentGapPram.SupervisionTimeoutMultiplier = p_enhanced_conn_complete->Supervision_Timeout;
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          UDSHandleNotification.ConnectionHandle = p_conn_complete->Connection_Handle;
          ETSHandleNotification.ConnectionHandle = p_conn_complete->Connection_Handle;
          RCSHandleNotification.ConnectionHandle = p_conn_complete->Connection_Handle;
          GHS_APP_EvtRx(&GHSHandleNotification);
          DIS_APP_EvtRx(&DISHandleNotification);
          BAS_APP_EvtRx(&BASHandleNotification);
          UDS_APP_EvtRx(&UDSHandleNotification);
          ...
          CurrentGapPram.MinConInt = p_conn_complete->Conn_Interval;
          CurrentGapPram.MaxConInt = p_conn_complete->Conn_Interval;
          CurrentGapPram.Latency   = p_conn_complete->Conn_Latency;
          CurrentGapPram.SupervisionTimeoutMultiplier = p_conn_complete->Supervision_Timeout;
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
    {
      ...
      switch (p_blecore_evt->ecode)
      {
        ...
        case ACI_GAP_PAIRING_COMPLETE_VSEVT_CODE:
        {
          ...
          /* Notification done only for trusted connection */
...
void APP_BLE_Procedure_Gap_Peripheral(ProcGapPeripheralId_t ProcGapPeripheralId)
{
  ...
  switch(ProcGapPeripheralId)
  {
    ...
    case PROC_GAP_PERIPH_ADVERTISE_NON_CONN_START_LP:
    {
      CurrentGapPram.AdvInt = paramA;
      break;
    }
    case PROC_GAP_PERIPH_CONN_PARAM_UPDATE:
    {
      CurrentGapPram.MinConInt = paramA;
      CurrentGapPram.MaxConInt = paramB;
      break;
    }
    default:
      break;
...
static void Ble_Hci_Gap_Gatt_Init(void)
{
  ...
  /* Set SC Key type to Debug */
  ret = aci_hal_write_config_data(CONFIG_DATA_SC_KEY_TYPE_OFFSET, CONFIG_DATA_SC_KEY_TYPE_LEN, &debug);
  if (ret != BLE_STATUS_SUCCESS)
  {
    ...
    case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
      {
        ...
        p_adv_data[i+6] = FW_ID_PROXIMITY; /* FW ID */
...
void APPE_Button1Action(void)
{
  ...
  if(iteration == 0)
  {
    ret = aci_gap_delete_ad_type(AD_TYPE_MANUFACTURER_SPECIFIC_DATA);

    if (ret != BLE_STATUS_SUCCESS)
    {
      LOG_INFO_APP("==>> aci_gap_delete_ad_type - Fail, result: %d \n", ret);
    }

    ret = aci_gap_update_adv_data(sizeof(a_ServiceData), a_ServiceData);

    if (ret != BLE_STATUS_SUCCESS)
    {
      LOG_INFO_APP("==>> aci_gap_update_adv_data - Fail, result: %d \n", ret);
    }
    else
    {
      LOG_INFO_APP("==>> aci_gap_update_adv_data - Success with Service Data \n");
    }
    iteration++;
  }
  else if(iteration == 1)
  {
    ret = aci_gap_update_adv_data(sizeof(a_ServiceDataUI2), a_ServiceDataUI2);
    ...
    {
      LOG_INFO_APP("==>> aci_gap_update_adv_data - Success with User Index 2 \n");
    }
    iteration++;
  }
  else if(iteration == 2)
  {
    ret = aci_gap_update_adv_data(sizeof(a_ServiceDataUI12), a_ServiceDataUI12);
    ...
    {
      LOG_INFO_APP("==>> aci_gap_update_adv_data - Success with User Index 1 and 2 \n");
    }
    iteration = 0;
  }
...
#else
  /* Update GHS Observation Schedule Changed */
  GHS_APP_UpdateObservationScheduleChanged();
  
  /* Update ETS Elapsed Time TZDSTOffset */
  uint8_t a_elapsedTime[9];
  
  /* Get current ETS Elapsed Time */
  ETS_APP_GetElapsedTime(a_elapsedTime);

  a_elapsedTime[8] += 1; /* TZDSTOffset = TZDSTOffset + 1; */
  
  /* Set new ETS Elapsed Time */
  ETS_APP_SetElapsedTime(a_elapsedTime);
  
  /* Generates 1 Live Health Observations and stores in DB */
...
#else
  extern uint8_t GHS_APP_LHOIndex;

  GHS_APP_LHOIndex ++;
  if(GHS_APP_LHOIndex > 8)
  {
    GHS_APP_LHOIndex = 0;
  }
  LOG_INFO_APP("Live Health Observations with Observation Class Type:%d \n", GHS_APP_LHOIndex+1);

  RCS_APP_UpdateFeature();
#endif
  return;
}
#endif
